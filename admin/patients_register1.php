...
file_put_contents('executed.txt', "Script hit at " . date('Y-m-d H:i:s') . "\n", FILE_APPEND);
file_put_contents('debug_request.txt', print_r($_REQUEST, true));
file_put_contents('debug_files.txt', print_r($_FILES, true));
file_put_contents('raw_input.txt', file_get_contents("php://input"));
file_put_contents('test_write.txt', "Write test at " . date('Y-m-d H:i:s'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // file_put_contents('C:/wamp64/www/ANGELORA/admin/debug_post.txt', print_r($_POST, true));
    ...
    $stmt = $pdo->prepare("SELECT email, phone, full_name FROM patients WHERE email = :email OR phone = :phone OR full_name = :full_name");
    $stmt->execute([':email' => $email, ':phone' => $phone, ':full_name' => $full_name]);
    $takenFields = [];
    ...
    try {
        ...
        $stmt_user = $pdo->prepare("INSERT INTO users (role_id, full_name, email, password, phone, created_at, profile_image) 
                                    VALUES (?, ?, ?, ?, ?, NOW(), ?)");
        $stmt_user->execute([$role_id, $full_name, $email, $password, $phone, $photoPath]);
        $user_id = $pdo->lastInsertId();

        $prefix = 'HSP';
        $year = date('Y');

        $stmt = $pdo->prepare("SELECT patient_id FROM patients WHERE patient_id LIKE ? ORDER BY patient_id DESC LIMIT 1");
        $stmt->execute([$prefix . '-' . $year . '-%']);
        $lastPatient = $stmt->fetch(PDO::FETCH_ASSOC);

        $nextNumber = ($lastPatient && preg_match('/HSP-' . $year . '-(\d+)/', $lastPatient['patient_id'], $matches))
            ? ((int)$matches[1] + 1)
            : 1;

        $patientId = $prefix . '-' . $year . '-' . str_pad($nextNumber, 4, '0', STR_PAD_LEFT);

        $stmt_patient = $pdo->prepare("INSERT INTO patients 
            (user_id, patient_id, full_name, gender, dob, age, email, phone, address, patient_pin, photo, patient_type, patient_status, hmo_name, language, marital_status, registered_by, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt_patient->execute([
            $user_id, $patientId, $full_name, $gender, $dob, $age, $email, $phone, $address,
            $pin, $photoPath, $patient_type, $patient_status, $hmo_name,
            $language, $marital_status, $registered_by
        ]);

        $pdo->commit();

        echo json_encode(['status' => 'success', 'pin' => $pin, 'patient_id' => $patientId]);
    } catch (Exception $e) {
        $pdo->rollBack();
...
$stmt = $pdo->query("SELECT user_id, full_name, role_id FROM users WHERE role_id IN (1)");
...
document.getElementById('patientForm').addEventListener('submit', function(e) {
  ...
  .then(data => {
    if (data.status === 'success') {
      ...
      modal.show();
      form.reset();
      setTimeout(() => window.location.href = "../index.php", 240000);
    } else {
      document.getElementById('response_message').innerHTML = `<div class='alert alert-danger'>${data.message}</div>`;
    }
  })
  .catch(() => {
