...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT patient_id FROM patients WHERE patient_id LIKE ? ORDER BY id DESC LIMIT 1");
        ...
        $stmt_patient = $pdo->prepare("INSERT INTO patients 
            (user_id, patient_id, full_name, gender, dob, age, email, phone, address, patient_pin, photo, patient_type, patient_status, hmo_name, ssn, language, registered_by, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt_patient->execute([
            $user_id, $patientId, $full_name, $gender, $dob, $age, $email, $phone, $address,
            $pin, $photoPath, $patient_type, $patient_status, $hmo_name,
            $ssn, $language, $registered_by
        ]);

        $pdo->commit();

        echo json_encode(['status' => 'success', 'pin' => $pin, 'patient_id' => $patientId, 'ssn' => $ssn]);
    } catch (Exception $e) {
        $pdo->rollBack();
    ...
    <form id="patientForm" enctype="multipart/form-data">
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-3">
          ...
          <input type="text" name="ssn" class="form-control" maxlength="11" required>
        </div>
        <div class="col-md-3">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-success shadow">
      ...
      <div class="modal-body text-center">
        ...
        <p><strong>SSN:</strong> <span id="modalSsn"></span></p>
...
document.getElementById('patientForm').addEventListener('submit', function(e) {
  ...
  .then(data => {
    if (data.status === 'success') {
      ...
      document.getElementById('modalSsn').textContent = data.ssn;
      ...
      form.reset();
      setTimeout(() => window.location.href = "../index.php", 13000);
    } else {
